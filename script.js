...
//       window.location.href = `modelindex.html?username=${encodeURIComponent(username)}`;
...
//       window.location.href = 'modelindex.html';
//   } else {
...
function nextPage() {
  const username = document.getElementById('username').value;
  if (username) {
      // Encode the username and navigate to modelindex.html with query parameter
      const encodedUsername = encodeURIComponent(username);
      window.location.href = `modelindex.html?username=${encodedUsername}`;
  } else {
      alert('Please enter a username.');
  }
}
...
function playAudio() {
  ...
  audioElement.play().catch(error => {
      ...
      // Optionally handle the failure here, maybe show a message to the user
  });
}
